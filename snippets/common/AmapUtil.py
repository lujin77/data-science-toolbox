    ...
    def deco_retry(f):
        ...
        def f_retry(*args, **kwargs):
            ...
            while mtries > 0:
                ...
                log.error(
                    "invoke func failed after Retry {t} times, return None as default".format(t=mtries))
...
def getWithRetry(url, param):
    ...
    if info == "OK":
        ...
        log.warn("{info}, sleep={t}, url={url} param={param}".format(
            info=info, url=url, param=param, t=wait_time))
        time.sleep(wait_time)
        raise RuntimeError
    else:
        log.warn("{info}, url={url} param={param}".format(
            info=info, url=url, param=param))
        raise RuntimeError
...
def postWithRetry(url, param, headers):
    ...
    if info == u"OK":
        ...
        log.warn("{info}, sleep={t}, url={url} ".format(
            info=info, url=url, t=wait_time))
        time.sleep(wait_time)
        raise RuntimeError
    else:
...
def move2road(road, point):
    ...
    destination = VincentyDistance(
        meters=distance).destination(origin, bear_degree)
    return destination
...
class BindRoad():
    ...
    def __mk_args(self, points):
        ...
        param = dict()
        if len(points) == 1:
            param['location'] = str(points[0].longitude) + \
                ',' + str(points[0].latitude)
        else:
            param['location'] = '|'.join(
                map(lambda x: str(x.longitude) + ',' + str(x.latitude), points))
            param['batch'] = 'true'
        param['key'] = AMAP_KEY
        param['poitype'] = 'all'
        param['radius'] = 300
    ...
    def __match_road_by_amap(self, points):
        ...
        resp = requests.get(
            "http://restapi.amap.com/v3/geocode/regeo", param).json()
        ret = []
        if resp['info'] == "OK":
...
class Around():
    ...
    def one(cls, point, radius=300, poiType=None):
        ...
        try:
            # 待自动重试的http get
            data = getWithRetry(
                "http://restapi.amap.com/v3/place/around", param).json()
            ...
            if data['info'] == "OK":
                ...
                if not pois or len(pois) == 0:
                    log.debug("Around by amap no result find, return [] as default, param={param}".format(
                        param=param))
                    return []
                else:
            ...
            log.error(
                "Around by amap request failed, return [], point={pt}".format(pt=point))
            return []
    ...
    def batch(cls, points, poiType=None):
        ...
        try:
            ...
            for chunck in chunks(points, 20):
                ...
                results = [obj.__parseBatchResult(
                    x) for x in AmapBatch.request(urls)]
            ...
            log.exception(
                "amap Around batch request failed, return [[]] as default, points={pts}".format(pts=points))
...
class Regeo():
    ...
    def getRoads(cls, point):
        ...
        try:
            ...
            log.error(
                "getRoads by amap request failed, return [] as default, point={pt}".format(pt=point))
            return []
    ...
    def getName(cls, point):
        ...
        try:
            ...
            log.exception(
                "getName by amap request failed, return empty as default, point={pt}".format(pt=point))
            return ""
    ...
    def getNameBatch(cls, points):
        ...
        for chunk in chunks(points, 20):
            ...
            try:
                ...
                # 出错的情况，此批次命名全部设置为空
                log.error(
                    "getNameBatch by amap request failed, return [[]] as default, point={pts}".format(pts=points))
                return map(lambda x: "", points)
        return out
...
if __name__ == "__main__":
    ...
    SIZE = 30
    result = Around.batch(
        [geopy.Point(39.9849500000, 116.3077580000) for x in range(SIZE)])
    assert len(result) == SIZE
    result = result[0]
